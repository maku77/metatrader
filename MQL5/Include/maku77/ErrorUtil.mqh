...
#property link "maku77.github.io"

namespace ErrorUtil {
    /**
     * Alerts the last error.
     */
    void AlertLastError(string funcName=NULL, int errorCode=-1) {
        ::Alert(ErrorDescWithCode(funcName, errorCode));
    }

    /**
     * Returns runtime error description and code.
     */
    string ErrorDescWithCode(string funcName=NULL, int errorCode=-1) {
        if (errorCode == -1) errorCode = ::GetLastError();
        string prefix = "";
        if (funcName != NULL) prefix = funcName + " ";
        return StringFormat("%sERROR(%d): %s", prefix, errorCode, ErrorDesc(errorCode));
    }

    /**    
     * Returns runtime error description.
     */
    string ErrorDesc(int errorCode=-1) {
        if (errorCode == -1) errorCode = ::GetLastError();
        switch (errorCode) {
            ...
            //--- User-Defined Errors
            default: if (errorCode >= ERR_USER_ERROR_FIRST && errorCode < ERR_USER_ERROR_LAST) {
                return StringFormat("User error %d", errorCode - ERR_USER_ERROR_FIRST);
            }
        }
